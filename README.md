...
`install` writes `statusLine.command` (this binary's path) and `env.FORCE_HYPERLINK=1` into `~/.claude/settings.json`, preserving every other key and the file's order, via a temp file and rename. It recognises its own entry exactly (no substring or basename matching) and the previous Node installer's entry only when the file at `~/.claude/statusline.js` carries that script's header. Any other existing `statusLine` is refused unless `--force`.
...
Every repo-controlled string is stripped of control, zero-width and bidirectional-override characters, and every URL is built from percent-encoded segments, before it reaches the terminal.
...
|---|---|
| `true` | 27.7 ms |
| `claude-statusline` (full render) | 39.4 ms |

Calling `usr\bin\bash.exe` directly skips the launcher and reads about 12 ms lower on both rows. A persistent daemon still needs a spawned client under the same launcher, and a minimal Go process that prints one line measured within about 1 ms of the full render. Not built.
...
| `FORCE_HYPERLINK=0` opt-out | anthropics/claude-code CHANGELOG.md | line 1060 | contract |
